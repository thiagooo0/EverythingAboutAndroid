package com.kowksiuwang.everythingaboutandroid.leetcode.n0009_maxvalue

import org.junit.Assert
import org.junit.Test

/**
 * 礼物的最大价值
 * 在一个 m*n 的棋盘的每一格都放有一个礼物，每个礼物都有一定的价值（价值大于 0）。你可以从棋盘的左上角开始拿格子里的礼物，并每次向右或者向下移动一格、直到到达棋盘的右下角。给定一个棋盘及其上面的礼物的价值，请计算你最多能拿到多少价值的礼物？
 *
 * 示例 1:
 *
 * 输入:
 * [
 *   [1,3,1],
 *   [1,5,1],
 *   [4,2,1]
 * ]
 * 输出: 12
 * 解释: 路径 1→3→5→2→1 可以拿到最多价值的礼物
 *  
 *
 * 提示：
 *
 * 0 < grid.length <= 200
 * 0 < grid[0].length <= 200
 * Created by GuoShaoHong on 2020/8/26.
 */
class MaxValueGiftTest {
    /**
     *
     */
    @Test
    fun test() {
        Assert.assertEquals(
            maxValue(
                arrayOf(
                    intArrayOf(1, 3, 1),
                    intArrayOf(1, 5, 1),
                    intArrayOf(4, 2, 1)
                )
            ), 12
        )
    }

    fun maxValue(grid: Array<IntArray>): Int {
        if (grid.isEmpty() || grid[0].isEmpty()) {
            return -1
        }
        val values = Array(grid.size) {
            IntArray(grid[0].size)
        }
        var max = 0
        for (i in grid.indices) {
            for (j in grid[0].indices) {
                if (i - 1 < 0 && j - 1 < 0) {
                    values[i][j] = grid[i][j]
                } else if (i - 1 < 0) {
                    values[i][j] = values[i][j - 1] + grid[i][j]
                } else if (j - 1 < 0) {
                    values[i][j] = values[i - 1][j] + grid[i][j]
                } else {
                    values[i][j] =
                        Math.max(values[i][j - 1], values[i - 1][j]) + grid[i][j]
                }
                max = Math.max(max, values[i][j])
            }
        }
        return max
    }
}